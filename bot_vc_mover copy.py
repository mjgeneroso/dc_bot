...
if not TOKEN:
    raise ValueError("Missing bot token. Set DISCORD_BOT_TOKEN in your environment.")
...
async def move_all(
    ...
    if not from_channel.members:
        await interaction.followup.send(
            "No members in the specified voice channel.", ephemeral=True
        )
        return
    logger.info(f"Executing /move_all from {from_channel.name} to {to_channel.name}...")
    await interaction.response.defer()
    for member in from_channel.members:
        await member.move_to(to_channel)
    logger.info("/move_all command executed successfully.")
    await interaction.followup.send(
...
async def mute_member(
    ...
    logger.info(f"Executing /mute_member for {member.name} for {hours} hours...")
    await interaction.response.defer()
    ...
    async def unmute_after_delay():
        ...
        await interaction.channel.set_permissions(member, overwrite=overwrite)
        logger.info(f"{member.name} has been unmuted.")
...
async def unreacted_members(
    ...
    non_reacted_members = [
        member.mention for member in role.members if member.id not in reacted_users
    ]

    response = (
        "\n".join(non_reacted_members)
        if non_reacted_members
        else "Everyone with the role has reacted!"
    )
...
async def notify_unreacted(
    ...
    async for message in channel.history(limit=100):
        if (now - message.created_at).total_seconds() <= 86400:
            recent_messages.append(message)

    if not recent_messages:
        await interaction.followup.send(
            "No messages found within the last 24 hours.", ephemeral=True
        )
        return
